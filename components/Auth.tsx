...
const Auth: React.FC<Props> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Erro de conexão ou banco de dados não configurado.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl border border-slate-100">
          <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
            <button 
              ...
              className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${isLogin ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
            >
              Entrar
            </button>
            <button 
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#2B59C3] text-white py-2.5 rounded-lg font-black text-[11px] shadow-lg shadow-blue-100 flex items-center justify-center gap-2 active:scale-95 transition-all mt-4 disabled:opacity-70 uppercase tracking-tight"
            >
              {isLoading ? (
